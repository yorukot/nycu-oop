...
namespace ns_opengl
{
	...
	void dTEXTURE::drawTexture( ) const
	{
		...
		int i;
		for (i=0;i<4; i++) {
			glTexCoord2f(cube_data[5*i], cube_data[5*i+1]); 
			glVertex3f(cube_data[5*i+2], cube_data[5*i+3], cube_data[5*i+4]);
		}
		glEnd();
	}
	...
	void dTEXTURE::drawTexture(float x, float y, float z, float sx, float sy, float sz) const
	{
		...
		for (i=0;i<4; i++) {
			glTexCoord2f( cube_data[5*i], 
                cube_data[5*i+1] ); 
			glVertex3f( 
                cube_data[5*i+2]*sx+x, 
                cube_data[5*i+3]*sy+y, 
                cube_data[5*i+4]*sz+z );
		}
